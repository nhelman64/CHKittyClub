...
Partial Class NewStory
    ...
    Protected Sub ButtonUpload_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles ButtonUpload.Click
        If Page.IsValid Then
            ...
            Try
                ' Save the username, kitty name, and story in the database, pending approval
                Dim cmd As New SqlCommand
                With cmd
                    .Connection = conn
                    .CommandType = Data.CommandType.Text
                    .CommandText = "INSERT INTO dbo.CHKC_Members ([MemberName], [CatName], [MemberEmail], [Birthday], [MemberStory], [OtherHandicap], [LastActivityDate])" & _
                                    "VALUES (@MemberName, @CatName, @MemberEmail, @Birthday, @MemberStory, @OtherHandicap, @LastActivityDate)"
                    ...
                    catName.Value = Server.HtmlEncode(TextBoxKittyName.Text)
                    catName.Direction = Data.ParameterDirection.Input

                    Dim memberEmail As SqlParameter = .Parameters.Add("@MemberEmail", Data.SqlDbType.VarChar, 50)
                    memberEmail.Value = Server.HtmlEncode(TextBoxEmail.Text)
                    memberEmail.Direction = Data.ParameterDirection.Input

                    Dim catBirthday As SqlParameter = .Parameters.Add("@Birthday", Data.SqlDbType.DateTime)
                    catBirthday.Value = Server.HtmlEncode(TextBoxBirthday.Text)
                    catBirthday.Direction = Data.ParameterDirection.Input

                    Dim memberStory As SqlParameter = .Parameters.Add("@MemberStory", Data.SqlDbType.Text, 8192)
                    memberStory.Value = Server.HtmlEncode(TextBoxStory.Text)
                    memberStory.Direction = Data.ParameterDirection.Input

                    Dim oH As Integer
                    If RadioButtonHasDisability.Checked = True Then
                        oH = 1
                    Else
                        oH = 0
                    End If
                    Dim otherHandicap As SqlParameter = .Parameters.Add("@OtherHandicap", Data.SqlDbType.Int, 4)
                    otherHandicap.Value = oH
                    otherHandicap.Direction = Data.ParameterDirection.Input

                    Dim lastActivityDate As SqlParameter = .Parameters.Add("@LastActivityDate", Data.SqlDbType.Date)
                    lastActivityDate.Value = Today
                    lastActivityDate.Direction = Data.ParameterDirection.Input
    ...
    As String
        ...
        AddToDatabase(Server.HtmlEncode(TextBoxUsername.Text), _
                      Server.HtmlEncode(TextBoxKittyName.Text), _
                      fileName, Server.HtmlEncode(oCap.Text))
    ...
    As Attachment
        ...
        If sFileName <> "" Then
            ...
            Dim attach As New Attachment(sFileName, MediaTypeNames.Image.Jpeg)
